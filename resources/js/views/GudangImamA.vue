<template>
  <div class="gudang-imam-a">
    <div class="header-bar">
      <h2>Daftar Barang - Gudang Imam A</h2>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        status: '',
        deskripsi: '',
        ruangan_id: 1 // Ruangan ID untuk Gudang Imam A
      }
    }
  },
  computed: {
  ...
  methods: {
    fetchBarang() {
      fetch('http://127.0.0.1:8000/api/barang?ruangan_id=1')  // Sesuaikan dengan API endpoint
        .then(res => res.json())
        .then(data => {
          this.barangList = data.filter(item => item.ruangan_id == 1);
        })
