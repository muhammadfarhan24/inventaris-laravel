...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        status: '',
        deskripsi: '',
        ruangan_id: 6 // karena ini Lantai
      }
    }
  },
  computed: {
  ...
  methods: {
    fetchBarang() {
      fetch('http://127.0.0.1:8000/api/barang?ruangan_id=6')
        .then(res => res.json())
        .then(data => {
          this.barangList = data.filter(item => item.ruangan_id == 6);
        })
