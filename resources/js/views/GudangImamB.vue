<template>
  <div class="gudang-imam-b">
    <div class="header-bar">
      <h2>Daftar Barang - Gudang Imam B</h2>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        status: '',
        deskripsi: '',
        ruangan_id: 2 // karena ini Gudang Imam A
      }
    }
  },
  computed: {
  ...
  methods: {
    fetchBarang() {
      fetch('http://127.0.0.1:8000/api/barang?ruangan_id=2')
        .then(res => res.json())
        .then(data => {
          this.barangList = data.filter(item => item.ruangan_id == 2);
        })
